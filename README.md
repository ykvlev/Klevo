...
- [x] Фаза 3 — прогноз клёва: `ml/features.py` (матрица признаков), `ml/score.py`
  (правило-скор 0–100 → `predictions`), `ml/train.py` (LightGBM→ONNX, ждёт метки из `catches`),
  `GET /api/spots/{id}/forecast`; ML-контур включён в прогноз (`ml-v1`, обучен на демо-метках,
  паритет C#/Python сверен; реальные метки — уловы из `catches`)
...
- [x] Фаза 4.5 — продакшн-веб (частично): карта точек (`/map.html`, Leaflet vendored, тёмные тайлы CARTO,
  ...
  лента уловов с фото и вердиктом правил (`/feed.html`, `GET /api/catches/feed`); мобильная адаптация
