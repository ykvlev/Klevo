...
- [x] Бэкфилл погоды и солунара 2023-01-01 … 2026-08-20 (все 5 точек): погода 30096 ч/точка (0 пропусков), солунар 1267 дня/точка (дозалив 2026-08-01…08-13)
...
- [x] CMEMS MY-реанализ (phy/bgc) обновляется с задержкой ~2.5 мес: phy ≤ 2026-05-31,
      bgc ≤ 2026-05-01; свежих данных нет. Для морских точек SST закрыт NASA
      (приоритет слияния: CMEMS → NASA). Chl-a bgc у Лахты пуст с 2025-01 (маска/порог)
- [x] CMEMS Chl-a (`cmems_mod_bal_bgc_my_P1M-m`, `--bgc`) — 65 записей (Лахта, Кургальский)

## Оценка устаревших CMEMS-полей: климатология + достоверность (реализовано)
Когда актуального спутникового значения нет (CMEMS запаздывает ~2.5 мес), API
показывает оценку по **климатологии** — среднее по тем же дням года из истории
точки (окно ±30 дней по DOY, с учётом перехода года), и помечает её как оценку.

- Класс `SatelliteEstimator` (`backend/src/Klevo.Api/SatelliteEstimator.cs`).
- Достоверность актуального значения (свежесть ≤ maxDays, где sst=30, chl=60,
  море/phy=45): `Conf = 70 + 25·(1 − stale/maxDays)` → 70…95%.
- Достоверность оценки: `Conf = 40 + 55·min(n/40, 1)·(1 − min(stale, 180)/180)`
  → 40…95% (n — число отсчётов климатологии, stale — дней с последнего снимка).
- В `/forecast`: `dataConfidence` (средняя по sst/chl/погоде/солунару),
  `sources[]` (по источникам: свежесть, staleDays, статус ok/warn/stale),
  `satellite[]` (оценки sst_c/chla_mgm3 с `estimated`, `confidence`, `basis`),
  `dataNote` — русская сводка «… климатология 2023–2025 (n=183) …».
- В `/conditions`: `satelliteSummary[]` (все 5 спутниковых фичей + `usedInModel`).
- ML-фичи не менялись, паритет C#/Python сохранён (модель не знает про оценки).
